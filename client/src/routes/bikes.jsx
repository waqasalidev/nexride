...
import { useBikes } from "@/lib/api";
import { useState, useEffect } from "react";

export const Route = createFileRoute("/bikes")({
    head: () => ({
        meta: [
            { title: "Superbikes — NexRide X" },
            { name: "description", content: "Track-bred superbikes, touring machines, and electric two-wheelers." },
        ],
    }),
    component: BikesPage,
});
...
function BikesPage() {
    ...
    const activeBikes = isMounted && dbBikes && dbBikes.length > 0 ? dbBikes : vehicles.filter((v) => v.category === "bike");

    const filteredList = activeBikes.filter((v) => {
    ...
    return (<>
      <PageHeader eyebrow="Inventory / Bikes" title={<>SUPERBIKES &<br /><span className="text-cyan-glow">TRACK MACHINES</span></>} description="Precision-engineered two-wheelers built for the apex."/>
